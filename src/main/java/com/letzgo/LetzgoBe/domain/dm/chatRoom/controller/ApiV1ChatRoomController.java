...
public class ApiV1ChatRoomController {
    ...
    public Page<ChatRoomList> getAllRooms(Pageable pageable, @LoginUser LoginUserDto loginUser){
        return chatRoomService.findAll(pageable, loginUser);
    }

    // 사용자의 채팅방 목록에서 검색(여기서 채팅방 생성 가능)
    @GetMapping("/chat-rooms/search")
    public Page<ChatRoomList> searchRooms(@RequestParam("keyword") String keyword, Pageable pageable, @LoginUser LoginUserDto loginUser){
        return chatRoomService.searchByKeyword(keyword, pageable, loginUser);
    }

    // 선택한 유저와 채팅방(1:1, 단체) 생성
    @PostMapping("/chat-room")
    public ResponseEntity<String> createChatRoom(@RequestBody @Valid ChatRoomForm chatRoomForm, @LoginUser LoginUserDto loginUser) {
    ...
    public ResponseEntity<String> inviteUser(@RequestBody @Valid ChatRoomForm ChatRoomForm, @LoginUser LoginUserDto loginUser) {
    ...
    public ResponseEntity<String> leaveChatRoom(@PathVariable("chatRoomId") String chatRoomId, @LoginUser LoginUserDto loginUser) {
